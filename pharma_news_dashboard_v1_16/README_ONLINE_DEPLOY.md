...
- v1.13 계열의 보관정책/보관기간 자동정리 파이프라인은 제거했습니다.
- 링크 복구 로직, 접힘형 진단 정보, 5개 탭 구조는 유지했습니다.
- 온라인 환경의 pandas dtype 오류를 줄이기 위해 DataFrame 정규화는 행 단위 재구성 방식으로 안정화했습니다.
- requirements.txt에서 pandas를 `>=2.2,<2.3`으로 제한했습니다.
