...
def looks_like_official_policy_change(row: pd.Series) -> bool:
    ...
    if category == "식약처/규제" and _has_any_text(text, MFDS_TERMS) and _has_any_text(text, POLICY_CHANGE_KEYWORDS):
        return True
    if _has_any_text(text, OVERSEAS_REGULATORS) and _has_any_text(text, POLICY_ACTION_KEYWORDS + POLICY_CHANGE_KEYWORDS):
        return True
    return False
...
def extract_policy_articles(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if out.empty:
        return out
    out["policy_type"] = out.apply(lambda r: policy_type(r.get("title", ""), r.get("summary", ""), r.get("article_text", "")), axis=1)
